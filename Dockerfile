...
SHELL ["/bin/bash", "-c"]
RUN cmake -DCMAKE_BUILD_TYPE='Release' -DCMAKE_PREFIX_PATH='/OpenMS/contrib-build/;/usr/;/usr/local' -DHAS_XSERVER=OFF -DBOOST_USE_STATIC=OFF -DOPENMP=ON ../OpenMS
SHELL ["mamba", "run", "-n", "streamlit-env", "/bin/bash", "-c"]
RUN cmake -DPYOPENMS=ON -DPY_MEMLEAK_DISABLE=On .
